...
| 7 | Matchmaking, ranking, colas competitivas | ⚠️ Arrancado (`matchmaking/`, servicio Python fuera de Godot) — emparejar hecho y probado en vivo (cola anónima, lanza un `ServerRoot` dinámico por partida). El servidor ahora también cierra su propio proceso solo al terminar la partida y el cliente muestra quién ganó en vez de un "se perdió la conexión" genérico — con un límite conocido documentado (bajo carga real, a veces un cliente ve igual el mensaje genérico; el cierre del proceso en sí siempre funciona). Sin cuentas/login/ranking todavía. Ver `docs/architecture/Implementation_Decisions.md` |
| 8 | Bots, IA | Pendiente |
| 9 | Steam | Pendiente |
...
objetivos de desbloqueo por ubicación/estructura — ver más abajo.
